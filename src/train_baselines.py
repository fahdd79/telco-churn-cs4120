...
print("INPUT FEATURES =", X_train.shape[1])  # ← add this line
class_models = {
    "LogisticRegression": Pipeline([("scaler", StandardScaler()), ("model", LogisticRegression(max_iter=2000))]),
    "DecisionTreeClassifier": Pipeline([("scaler", StandardScaler()), ("model", DecisionTreeClassifier(random_state=42))]),
}
...
for name, pipe in class_models.items():
    with mlflow.start_run(run_name=f"class_{name}", nested=True):
        ...
        try:
            auc_v = roc_auc_score(y_class_val, pipe.predict_proba(X_val)[:, 1])
        except Exception:
            auc_v = np.nan
        ...
        try:
            auc_t = roc_auc_score(y_class_test, pipe.predict_proba(X_test)[:, 1])
        except Exception:
            auc_t = np.nan
...
table_cls = pd.DataFrame(
    rows_cls,
    columns=["Model","Accuracy_val","F1_val","ROC_AUC_val","Accuracy_test","F1_test","ROC_AUC_test"]
)
...
reg_models = {
    ...
    "DecisionTreeRegressor": Pipeline([("scaler", StandardScaler()), ("model", DecisionTreeRegressor(random_state=42))]),
}
...
plt.tight_layout()
plt.savefig(PLOTS_DIR / "plot4_residuals_vs_predicted.png")
plt.clf()
